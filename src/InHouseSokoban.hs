...
getLogicalEvents e = LogicalEvents $ any
  (\x ->
...
nextDir (x : xs) = case x of
  (SDL.KeyboardEvent a) ->
    if SDL.keyboardEventKeyMotion a == SDL.Pressed && not
         (SDL.keyboardEventRepeat a)
    ...
    let
      ...
      layers =
        [ map
            (\l@(Location x y) ->
              ( l
              , StillInstance "floor_tiles"
                              (zoomRect (Rect (16 * x) (16 * y) 16 16) scale)
                              0
              )
            )
          $ pFloorTiles sp
        , map
            (\l@(Location x y) ->
              ( l
              , StillInstance "dark_floor_tiles"
                              (zoomRect (Rect (16 * x) (16 * y) 16 16) scale)
                              0
              )
            )
        ...
        , ( map
          ...
          ++ ( map
                 (\l@(Location x y) ->
                   ...
                   , AnimationInstance
                     ("char_idle_" ++ dirStr)
                     (zoomRect (Rect (16 * x) (16 * y - 3) 16 16) scale)
...
outside (Location u v) walls =
  not (any (\(Location x y) -> u < x && y == v) walls)
    || not (any (\(Location x y) -> u > x && y == v) walls)
    || not (any (\(Location x y) -> v < y && x == u) walls)
    || not (any (\(Location x y) -> v > y && x == u) walls)
  ...
  let compX f (Location x1 y1) (Location x2 y2) = y1 == y2 && f x1 x2
      ...
      m        = map
        (\l@(Location x y) ->
          ...
          in  ( l
              , case undefined of
                _ | nw left && nw up && outside' left -> 4
        ...
        walls
  in  (\[a, b, c, d, e, f, g, h, i, j, k] -> (a, b, c, d, e, f, g, h, i, j, k))
        $ map (map fst)
        $ (\l -> l ++ (take (11 - length l) $ repeat []))
        $ reverse
        $ snd
        $ foldl
            (\(c, l) g@((_, i) : _) ->
              (c + 1, (g : (take (i - c) $ repeat [])) ++ l)
            )
            (0, [])
        $ groupOn snd
        $ sortOn snd m
...
getStaticPlacements b =
  let
    ...
    (boxesOnT, boxesOffT) = foldl
      ...
      boxes
    (xm, ym) =
      foldl (\(a, b) (Location x y) -> (max a x, max b y)) (0, 0) walls
    outside' = flip outside walls
    floor =
      [ Location x y
      | x <- [0 .. xm]
      , y <- [0 .. ym]
      , not $ outside' $ Location x y
      ]
    darkFloor =
