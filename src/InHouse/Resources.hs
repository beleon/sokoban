...
createTexturesFromConf conf renderer =
  ...
  in  foldlM
        (\m [fn, n, a] ->
          fmap (flip (M.insert n) m) $ createTexture renderer fn $ parseColor a
        )
        M.empty
        textureDefinitions
      ...
      instances =
        map
            (\x -> case x of
              ...
              _            -> error "Wrong number of instance arguments"
            )
          $ fromJust
          $ getConf "instances" conf
...
toSDLRectangle (Rect x y w h) = SDL.Rectangle
...
createTexture renderer name (r, g, b) = do
  ...
  SDL.surfaceColorKey surface
    SDL.$= ( Just
           $ SDL.V4 (fromIntegral r) (fromIntegral g) (fromIntegral b) 0x00
           )
