...
drawFrame renderer resources (Frame layers) = mapM_ (mapM_ (drawSpriteInstance renderer resources)) layers
  ...
  let (texture, from, to) = case si of
        (StillInstance sId sLoc sInd) ->
          let (Still _ textureId (w, h) instances) = rSprites resources M.! sId
              ...
              ((x, y), off) = second ((* scale) . fromTuple . fromMaybe (0, 0)) (instances!!sInd)
          in (texture, Rect x y w h, moveRect sLoc off)
        (AnimationInstance aId aLoc aTicks) ->
          let (Animation _ textureId (w, h) off frames dur) = rSprites resources M.! aId
              texture = rTextures resources M.! textureId
              (V2 toW toH) = dim aLoc
              scale = V2 (mod toW w) (mod toH h)
          ...
          in (texture, Rect x y w h, moveRect aLoc off')
