...
loadBoard s =
  toBoard $ constructBoardBuilder (BoardBuilder [] [] Nothing) s $ Location 0 0
...
constructBoardBuilder bb ('\n' : es) l =
  constructBoardBuilder bb es $ Location 0 $ inc $ l ^. y
...
updateEntityLocation b l1 l2 =
  obstacles %~ (uncurry (M.insert l2) . ((M.! l1) &&& (M.delete l1))) $ b
...
playGame' [] _ = []
playGame' (e : es) s =
  let mr = advance s e in mr : playGame' es (mr ^. sokoban')
...
printGame s =
  let
    ...
    nextL l =
      if l ^. x < xLen - 1 then x %~ inc $ l else Location 0 (1 + (l ^. y))
    isT l = l `elem` t
